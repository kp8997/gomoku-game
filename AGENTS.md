...
4. **Mandatory Feature Plans:** Every suggestion, new feature, or implementation plan MUST be created as a new file in the `ai-feature-plan/` directory (following the `{NN}-ai-{feature-name}-feature-plan.md` format). When in Planning Mode, do NOT save the plan in a temporary artifact folder; instead, write the implementation plan content directly into a new file in this `ai-feature-plan/` folder. This must be done BEFORE executing any code changes so we can permanently track and analyze every implementation plan.
